...
// Синхронізація застарілих полів рівня замовлення з ПЕРШОЮ посилкою.
//
// ⚠️ Тимчасова конструкція, і саме тому вона тут одна на всіх. На order.ttn досі
// спираються крон трекінгу (Етап 2) та {ttn} у шаблонах і Telegram (Етап 3).
// Поки вони живі, перша посилка мусить бути видимою по-старому — інакше
// відстеження й повідомлення клієнту тихо перестануть працювати.
//
// ⚠️ npStatus* НЕ чіпаємо: їх пише крон на рівні замовлення, і в Етапі 1 він
// про посилки ще не знає. Стерти їх тут означало б зробити вигляд, що статусу
// немає, хоча він є.
function syncLegacyTtn(order) {
  const перша = (order.parcels || [])[0] || null;

  order.ttn = перша ? перша.ttn : null;
  order.ttnRef = перша ? перша.ttnRef : null;

  // ⚠️ Статус теж дзеркалимо. Відколи трекінг пише його В ПОСИЛКУ, ці поля
  // ніхто б не оновлював — а на них спирається пігулка статусу в СПИСКУ
  // замовлень. Без цього рядка список замерз би на тому, що встиг записати
  // старий крон, і показував би «в дорозі» біля давно отриманої посилки.
  //
  // Для замовлення з кількох коробок це статус ПЕРШОЇ: у списку є лише один
  // рядок, а повна картина — у картці, де кожна посилка має свою пігулку.
  order.npStatusCode = перша ? перша.npStatusCode || null : null;
  order.npStatusText = перша ? перша.npStatusText || null : null;
  order.npStatusUpdatedAt = перша ? перша.npStatusUpdatedAt || null : null;
}
