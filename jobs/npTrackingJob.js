...
async function позначитиДоставленим(order) {
  // ⚠️ Вже закрите замовлення не «переводимо» вдруге. isTransitionAllowed на
  // from === to відповідає true (це дозвіл зберегти картку без зміни статусу),
  // тож без цієї перевірки кожен прохід дописував би в журнал «Доставлено →
  // Доставлено» — і картка перетворилась би на стрічку з однакових записів.
...
async function runNpTracking() {
  ...
  try {
    ...
    // зламаний трекінг.
    //
    // ⚠️ «Доставлено» ТЕЖ беремо. Закінчився шлях ЗАМОВЛЕННЯ, а не посилки:
    // менеджер міг закрити картку раніше, ніж клієнт забрав коробку (до появи
    // сторожа в updateOrderById це дозволялось). Коробка після цього їде далі,
    // а її статус лишався замороженим назавжди — саме так замовлення показувало
    // «Доставлено» поруч із «Не передано». Що саме питати, вирішує стан
    // ПОСИЛКИ нижче, а не стан замовлення.
    //
    // «Скасовано» не беремо: там посилку зазвичай скидають, а не везуть.
    //
    // Автоперехід у «Доставлено» з «Оплачено» не станеться — isTransitionAllowed
    // пускає лише сусідні статуси, тож замовлення спершу мусить пройти
    // «Відправлено».
    ...
    // ⚠️ Питаємо лише про ті коробки, чий шлях ЩЕ не завершився.
    //
    //   • мертві (видалена / номер не знайдено) — статус більше не зміниться;
    //   • отримані (9/10/11) — клієнт забрав, далі нічого не буде.
    //
    // Відсіювання отриманих стало обов'язковим, відколи у вибірку входить
    // «Доставлено»: без нього кожне закрите замовлення поверталось би в пачку
    // щогодини й назавжди, а пачка — це 100 місць, відібраних у коробок, які
    // справді їдуть.
    const активні = [];
    ...
    orders.forEach(order => {
      (order.parcels || []).forEach(parcel => {
        ...
        const завершена = мертва || isParcelDelivered(parcel);

        if (parcel.ttn && !завершена) {
