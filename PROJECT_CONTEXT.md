...
- `acquiringClient = axios.create({ baseURL: process.env.ACQUIRING_BASE_URL })` — окремий інстанс
  (кожна інтеграція — власний axios-клієнт, див. розділ 7);
...
**Нові поля Order:** ~~`ttn`~~ (прибрано в Етапі 3 — номери живуть у
`parcels[]`; «Відправлено» вимагає посилку, а не це поле), `prepaymentAmount` (Number, null — 20% від
...
## 6.5. Лічильники непереглянутого (`isViewed`)

Поле `isViewed: Boolean, default: false` у трьох моделях: `Order`,
`Print3dOrder`, `FeedBack`. Це «менеджер уже бачив цей запис», і воно живе на
сервері саме тому, що раніше жило в localStorage адмінки: рефреш, другий
пристрій і другий менеджер мали кожен свою правду.

Ендпоінти (усі `authAdm`):

| Метод | Шлях | Що робить |
|---|---|---|
| GET | `/api/adm/counters` | `{ orders, print3d, feedback }` — по кожній колекції |
| PATCH | `/api/adm/orders/:numberOfOrder/viewed` | позначає замовлення |
| PATCH | `/api/adm/print3d/:id/viewed` | позначає заявку на 3D-друк |
| PATCH | `/api/adm/feedback/:id/viewed` | позначає заявку з форми |

⚠️ **Рахуємо `{ isViewed: { $ne: true } }`, а не `{ isViewed: false }`.** У
MongoDB документ, де поля НЕМАЄ, під `false` не підпадає: записи, створені до
появи поля, тихо випали б із лічильника — і при цьому світились би в списках
адмінки як нові. Цифра й екран розійшлись би.

⚠️ **`markViewed` повертає `alreadyViewed` — стан ДО запису.** Адмінка зменшує
лічильник локально, без повторного `/counters`; без цієї відповіді другий виклик
по тому самому запису віднімав би ще одиницю, хоч у базі нічого не змінилось.
А другий виклик — не рідкість: подвійний клік, дві вкладки, подвійний виклик
ефекту в React StrictMode. Технічно це `findOneAndUpdate` БЕЗ `{ new: true }` —
драйвер віддає документ у стані до оновлення.

⚠️ **Позначання — окремі ручки, а не side-effect у `getOrderById`.** Читання не
має писати в базу: інакше службовий захід у картку (перевірити ТТН, скопіювати
текст) знімав би позначку «нове» із замовлення, якого менеджер не розбирав.

⚠️ **`isViewed` треба тримати у whitelist-ах `getOrders` і `getFeedback`.** Ці
два контролери збирають відповідь із явного переліку полів (див. розділ 7), тож
без ручного додавання поле просто не доїде.

### Разовий скрипт `scripts/mark-existing-viewed.js`

Проставляє `isViewed: true` тому, що вже лежало в базі, — інакше лічильник
показав би всю історію магазину як «нове».
...
⚠️ **`--before` обов'язковий, дефолту «зараз» немає навмисно.** Скрипт
запускається руками, а бек уже приймає замовлення: між деплоєм і запуском може
прийти справжнє нове замовлення, і «оновити все» тихо поховало б його — воно
стало б переглянутим і ніколи не потрапило б у лічильник. Фільтр подвійний:
`isViewed: { $exists: false }` І `createdAt < cutoff`; запис, створений після
деплою, має `isViewed: false` (дефолт схеми) і під `$exists: false` не підпадає
навіть при помилковому cutoff. Ідемпотентний: другий запуск оновлює 0 записів.

---

## 6.6. Дашборд: «Замовлення в роботі»

`aggregateOrdersInWork` віддає `firstItemName` (назва ПЕРШОЇ позиції) і
`itemsCount`. Увесь `cartItems` у дашборд не тягнемо: рядок веде в картку
замовлення, повний склад там і є, а масив позицій у 15 рядках роздув би
відповідь на порядок.

⚠️ **`$arrayElemAt`, а не `$first`.** `$first` як оператор масиву з'явився лише
в MongoDB 4.4. На старішому сервері агрегація впала б цілком — а разом з нею
ВЕСЬ дашборд, бо блоки рахуються одним `Promise.all`.

---

## 6.7. Порядок товарів у списку (`order`)

Поле `order: Number, default: null` у `Product` і `ProductZbirky` — позиція в
СВОЄМУ списку. Менший order — вище.

⚠️ **«Категорія» тут — це список, який бачить менеджер, а не поле в базі.**
Акумулятори віддаються запитом за `type` ('18650'), решта за `category`, і все
це у двох різних колекціях. Перелік областей — `helpers/productScopes.js`, один
на ендпоінт reorder і на разовий скрипт: розійдись вони, порядок у базі перестав
би відповідати екрану.

| Метод | Шлях | Що робить |
|---|---|---|
| PUT | `/api/adm/products/reorder` | `{ scope, orderedIds }` → `bulkWrite`, `order = індекс` |

Ендпоінт перевіряє, що всі id належать цьому списку: без перевірки бага в UI
тихо проставила б чужим товарам порядок сусідньої категорії.

⚠️ **Самого лише `.sort({ order: 1 })` НЕ досить.** У MongoDB відсутнє поле й
null сортуються ПЕРЕД числами, тож товар, доданий повз адмінку, стрибнув би на
перше місце вітрини. База дає базовий порядок (`ORDER_SORT` = order asc, потім
_id asc), а `sortByOrder()` стабільним поділом переносить невпорядковані в хвіст.
`_id` другим ключем обов'язковий: поки order однаковий, сортування за одним
полем не визначене і сітка стрибала б між перезавантаженнями.

⚠️ **Змішані списки за order НЕ сортуються навмисно** — усі акумулятори
(`/batteries`), популярне, акції. order унікальний у межах свого списку, тож у
збірному переліку товари різних типів мали б однакові номери й перемішались би.
В адмінці спільної сторінки для них немає, а отже й порядку, який треба повторити.

Новий товар отримує `order = max + 1` у своєму списку (`getNextOrder`) — тобто
стає останнім, а не з'являється в випадковому місці вітрини.

### Разовий скрипт `scripts/init-product-order.js`
...
Читає товари ТИМ САМИМ запитом без сортування — тобто бачить рівно те, що
бачить сайт сьогодні, — і проставляє 0,1,2… Візуально не змінює нічого.
Запобіжник: пропускає списки, де order уже заданий (`--force` знімає) — інакше
повторний запуск переписав би ручний порядок назад на натуральний.

---

## 6.8. Фото товару: контракт keepImages + files

`image` лишається масивом URL-рядків Cloudinary (формат навмисно не мігрували).
`image[0]` — обкладинка в каталозі, тому порядок значущий.

Запит описує **підсумок**, а не дію:

| Поле | Що означає |
|---|---|
| `keepImages` | наявні фото, які ЛИШАЮТЬСЯ (можна кілька полів, або JSON-масив) |
| `files` | нові файли (multer) |
| `imageOrder` | фінальний порядок: URL для наявних, `new:<індекс>` для нових |

Раніше було «прийшли файли — перезаписати `image` цілком», і додати одне фото
до чотирьох наявних було неможливо в принципі.

⚠️ **`imageOrder` існує заради одного сценарію**: менеджер додає фото й одразу
тягне його на місце обкладинки. Без нього бек ставить нові в кінець, і фото,
яке в інтерфейсі вже стояло першим, після збереження стрибало б униз.

⚠️ **Сумісність**: якщо `keepImages` НЕ передано — стара поведінка (файли
замінюють усе). Ознака нового контракту саме присутність поля, тож вкладка зі
старою адмінкою продовжує працювати до заливки нової збірки.

⚠️ **Видалення сиріт із Cloudinary** (`cleanupOrphans`) має два запобіжники:
фото, яке ще використовує ІНШИЙ товар, не чіпаємо; будь-який збій видалення
ковтається — втратити файл у хмарі не так дорого, як не зберегти товар.

⚠️ **Ліміти живуть в ОКРЕМОМУ multer** (`middlewares/uploadProductImages.js`,
15 файлів × 10 МБ, лише `image/*`) і застосовані тільки до 4 продуктових
роутів. Спільний `upload` чіпати не можна: він обслуговує форму 3D-друку, куди
клієнти вантажать .stl і .zip.

`extractPublicId(url)` дістає id для видалення: усе після сегмента версії
(`v123`) без розширення, разом із текою (`images/name`).

---

## 7. Важливі рішення й нюанси (неочевидне з коду)

⚠️ **Ручна знижка менеджера живе в `personalDiscountRate`/`personalDiscountValue`
і МУСИТЬ бути в схемі.** Адмінка надсилала ці поля з самого початку, а схема їх
не мала — Mongoose мовчки їх викидав. Далі ланцюжок: у відповіді їх немає →
`usePromoCode` в адмінці рахує знижку заново як нуль → переписує `together` на
суму без знижки → НАСТУПНА зміна статусу пише цю суму в базу. Клієнту вже
назвали 10 000 ₴, а замовлення тихо ставало на 12 645 ₴. Виправлено 2026-09-09.

⚠️ **`prepaymentAmount` перераховується лише коли змінилась сама сума** і лише
поки замовлення в «Нове»/«Очікує оплати» (`updateOrderById`). Рахувати на кожне
збереження не можна: значення навмисно заморожене, щоб пізніша зміна відсотка в
налаштуваннях не рухала суми у вже виставлених замовленнях.

⚠️ **До оплати це поле — прогноз, після оплати — факт** (2026-09-14). Значення з
тіла запиту приймається рівно в одному випадку: накладений платіж **і** перехід
у «Оплачено» **і** поточний статус не «Оплачено». Тоді туди лягає сума, яку
клієнт РЕАЛЬНО вніс, — менеджер вводить її в картці при підтвердженні оплати.
Причина: клієнти регулярно переказують не те, що їм виставили (рівну тисячу
замість 2 529 ₴), і решта накладеним має рахуватись від факту.

Від цього числа далі рахується все: решта в картці адмінки, `{codAmount}` у
тексті для клієнта (`helpers/renderTemplate.js`) і сума накладеного платежу в
ТТН. Тому окремого поля «скільки оплачено» не заводили — була б друга правда.

⚠️ Двері вузькі навмисно: поза цим переходом поле з тіла видаляється. Адмінка
шле назад увесь об'єкт замовлення, тож без цієї умови кожне збереження
переписувало б передоплату тим, що випадково лежало в сторі. І факт переважає
перерахунок відсотком: якщо в тому ж запиті змінилась `together`, внесена сума
все одно не затирається.

### Лічильники вкладок у списку клієнтів (2026-09-15)

`GET /adm/contacts` віддає `counts: { all, site, manual }`.

⚠️ Рахуються з фільтра БЕЗ типу. Питання на кнопці — «скільки з сайту серед
того, що я зараз шукаю», а не «скільки з сайту в поточній вибірці»: друге на
сусідніх вкладках завжди давало б 0. Пошук і `kind` лічильники звужують,
вибрана вкладка — ні.

⚠️ `total` лишається по вибраній вкладці: він потрібен пагінації й рядку
«Показано X із Y».

### Воронка: Підхід А, Реанімація, хронологія (2026-09-15)

⚠️ **У воронці лише ті, хто ЩЕ НЕ купив** — `isPotential`: (manual АБО wholesale)
І 0 замовлень. Перше замовлення виводить клієнта з дошки назавжди. Воронка про
шлях ДО першої покупки; хто вже купує, живе у списку клієнтів.

⚠️ **Реанімація** — окремий вхід: оптовик, який мовчить 14–60 днів (`isSleeping`).
До 14 днів людина ще не встигла, після 60 відносини розірвані. Тільки опт:
роздрібний купує раз на рік, і «мовчання» в нього нічого не означає.

⚠️ **Склад Реанімації рахується НА ЛЬОТУ з дат замовлень**, а не зі збереженої
стадії. Тому сплячий показується в «Реанімації» незалежно від `funnelStage`, а
перетягнути туди картку адмінка не дає.

⚠️ **Запис під час читання** (`позначитиРеанімацію` в `getFunnel`) — свідомий
виняток із правила «читання не пише в базу» (пор. `markViewed`). «Замовк» — не
подія, а її відсутність: жоден виклик не настає в момент, коли клієнт перестав
замовляти, а cron у проєкті немає. `reactivationSince` — сторож одноразовості:
без нього кожне відкриття дошки затирало б `nextContactDate` менеджера.
Вихід з епізоду знімає обидва поля.

⚠️ `startingStage` і `scripts/init-funnel-stages.js` ВИДАЛЕНІ: клієнт, що входить
у воронку, за визначенням не має замовлень, тож рахувати нічого.

### Охоплення трекінгу (2026-09-18)

Два способи, якими статус посилки міг завмерти назавжди. Обидва відтворені
живими запитами до Пошти на бойових номерах.

⚠️ **Питаємо про те, чий шлях ще НЕ завершився — і це про ПОСИЛКУ, а не про
замовлення.** Вибірка: статуси `Оплачено / Відправлено / Доставлено`, а
відсіювання — по стану самої коробки (мертві 2/3 і отримані 9/10/11 не
перепитуємо). «Доставлено» входить навмисно: менеджер міг закрити картку
раніше, ніж клієнт забрав коробку, і далі її статус лишався замороженим
назавжди — саме так замовлення показувало «Доставлено» поруч із «Не передано».
Без відсіювання отриманих кожне закрите замовлення поверталось би в пачку
щогодини й довічно.

⚠️ **`позначитиДоставленим` виходить одразу, якщо замовлення ВЖЕ «Доставлено».**
`isTransitionAllowed` на `from === to` відповідає true (це дозвіл зберегти
картку без зміни статусу), тож без цієї перевірки кожен прохід дописував би в
журнал «Доставлено → Доставлено».

⚠️ **Прохід дивився ЛИШЕ на «Відправлено».** Посилки пакують уже з «Оплачено», і
накладна починає рухатись у Пошті з моменту створення — вона не чекає, доки
менеджер натисне «Відправити». До того посилка лишалась із останнім відомим
кодом, найчастіше «Не передано». Тепер у вибірці `{ $in: [PAID, SHIPPED] }`.
**Перестрибнути в «Доставлено» з «Оплачено» не вийде**: `isTransitionAllowed`
пускає лише сусідні статуси — перевірено тестом.

⚠️ **Зворотний пошук ішов за СИРИМ `parcel.ttn`, а запит — за обрізаним.** Номер,
що зберігся з пробілом, Пошту опитував, але відповідь до нього вже не
приліплювалась: помилки немає, у лозі порожньо, статус просто не оновлюється
ніколи. Тепер нормалізовані обидві сторони, і `ttn` у схемі має `trim: true`.

⚠️ **`GET /adm/np/tracking-status`** — коли прохід був востаннє і чим закінчив.
Крон живе ВСЕРЕДИНІ веб-процесу: коли той спить, перезапускається чи падає,
прохід не стається мовчки, і зовні це не відрізнити від «Пошта не оновлює
статуси». Адмінка показує це в Налаштуваннях.

### Сторожі переходів і отримувач посилки (2026-09-18)

⚠️ **«Відправлено» вимагає накладну лише там, де щось ВЕЗУТЬ.** Спільна вимога
посилки стосувалась і самовивозу — а накладної для нього не існує, тож такі
замовлення не виходили з «Оплачено» ніколи.
...
однією канонічною назвою тихо зняло б вимогу ТТН з усіх них — найгірший
різновид помилки: нічого не ламається, просто перевірки більше немає. Невідомий
тип доставки лишається суворим.

⚠️ **«Доставлено» РУКАМИ — лише коли Пошта підтвердила ВСІ посилки.** Це
кінцевий стан: назад дороги немає, він ставить `deliveredAt` і закриває
статистику. Замовлення БЕЗ посилок не блокуємо — такі лишились із часів до
посилок і не закрились би ніколи.

⚠️ **Автоперехід крона сторожа НЕ проходить**: `jobs/npTrackingJob.js` пише
через `order.save()` і в `updateOrderById` не заходить. Якби гвардія зачепила
його, замовлення перестали б закриватись самі — і помітили б це не одразу.

⚠️ **ПІБ отримувача — ТРИ поля** (`lastName`, `firstName`, `middleName`).
Раніше приходив один рядок, який ділився по пробілу «прізвище — решта», тож
«Іван Петренко» давало прізвище Іван: накладна на неіснуючу людину, і
з'ясовувалось це у відділенні. `name` бек складає сам із трьох полів; старий
однорядковий `name` ще приймається (`розібратиПІБ`) — адмінка викладається
окремо.

⚠️ **`toUaPhone` (`helpers/phone.js`) — дзеркало `phoneSchema.ts` на фронті.**
Нуль-транк (067…) прибирається, а не відхиляється. Перевірка присутності полів
іде ПЕРЕД перевіркою формату — щоб півзаповнена адреса давала одне зрозуміле
повідомлення, а не водила менеджера по формі кругами.

⚠️ **Наложка на посилці — лише для «Накладений платіж».** Раніше вона їхала в
Пошту незалежно від способу оплати, тобто з уже оплаченого замовлення могли
зібрати гроші вдруге.

### Посилки — Етап 3, фінал (2026-09-17)

Посилки стали ЄДИНИМ джерелом правди про накладні. Старих полів у коді немає.

⚠️ **`{ttn}` — це СПИСОК номерів через кому, а не один.** Плейсхолдер лишили з
тією самою назвою навмисно: власник уже написав під нього свої шаблони, і новий
`{ttnList}` означав би, що всі вони мовчки перестануть показувати номер.
Посилок немає → рядок зникає цілком (правило `renderTemplate`).

⚠️ **Поля прибрані З КОДУ, але НЕ з бази** (Варіант А). `ttn`, `ttnRef`,
`npStatusCode/Text/UpdatedAt` лишаються в документах — їх ніхто не читає, і
видалення колонок необоротне. Наслідок: Mongoose вирізає їх із кожного читання,
тому `scripts/migrate-ttn-to-parcels.js` працює через СИРУ колекцію
(`connection.db.collection('orders')`) — інакше він на повній базі рапортував би
«нічого мігрувати».

⚠️ **`create-ttn` і `reset-ttn` ВИДАЛЕНІ.** Ручний ввід номера писав `order.ttn`
без посилки, тобто замовлення їхало повз облік коробок. Усе роблять
`/parcels` і `/parcels/:id/reset`.

⚠️ **«Відправлено» тепер вимагає посилку з номером**, а не поле `ttn`:
`(current.parcels || []).some(p => p.ttn)`.

⚠️ **ПОРЯДОК ДЕПЛОЮ: адмінка ПЕРША, бек ДРУГИЙ.** Нова адмінка не ходить у
видалені роути й сама рахує статус із `parcels`; стара — ходить. Зворотний
порядок лишає менеджера з кнопками, що дають 404.

⚠️ **Зведений статус рахує АДМІНКА** (`getOrderNpSummary`), бо вона деплоїться
першою: Проблема > Доставлено > Частково X з Y > В дорозі > Очікує статусу.
Бек лише віддає `parcels` у `getOrders`.

⚠️ **Публічний `get-order` віддає БІЛИЙ список полів.** Раніше — весь документ
мінус короткий чорний список, тож кожне нове поле (внутрішня примітка, габарити
посилок, звʼязок із CRM) потрапляло клієнту в день появи. Забути додати поле в
білий список видно одразу — значення зникло в кабінеті; забути в чорний —
непомітно назавжди.

### Трекінг посилок — Етап 2 (2026-09-17)

Крон читає `parcels`, а не `order.ttn`. Статус пишеться В ПОСИЛКУ.

⚠️ **`$elemMatch`, а не `'parcels.ttn': { $nin: [...] }`** — перевірено запитом:
друга форма затягує ще й замовлення з ПОРОЖНІМ масивом посилок (шлях до
неіснуючого поля під `$nin` вважається збігом).

⚠️ **Мертві посилки (коди 2/3) у запит не потрапляють.** Їхній статус уже не
зміниться, а місця в пачці відбирались би в коробок, які справді їдуть.

⚠️ **«Доставлено» — лише коли ВСІ посилки отримані** (`allParcelsDelivered`).
Замовлення з двох коробок, де приїхала одна, — це замовлення в дорозі.
**Наслідок:** мертва посилка НЕ рахується отриманою, тож замовлення з нею не
закриється саме — це навмисно, розібратись має людина (скинути й створити нову).

⚠️ **Дзеркала більше немає** — `syncLegacyTtn` прибрано в Етапі 3 разом із
`order.npStatus*`. Пігулку в списку тепер рахує адмінка з `parcels`.

⚠️ Журнал каже, ЯКА посилка зрушила: «Посилка {ttn}: {статус}». Telegram про
повернення теж називає конкретну коробку — її ТТН, товари і, якщо є, іншу адресу.

`cronLock`, розклад 7–21 Київ, пачка ≤100, стійкість до збоїв — без змін.

### Посилки замовлення — Етап 1 (2026-09-17)

`order.parcels[]` — замовлення може їхати кількома коробками. Кожна: свої товари,
своя адреса, своя наложка, свій номер.

⚠️ **Позиція посилається на РЯДОК кошика (`lineIndex`), не на `codeOfGood`.** Той
самий код трапляється в кошику двічі — з різною ємністю, герметизацією чи
холдером, — і це РІЗНІ товари. Розподіл за кодом злив би їх і дозволив
відправити «чотири», коли в кожному рядку по дві. Індекс безпечний: кошик
редагується лише в «Нове»/«Очікує оплати», а посилки — з «Відправлено».

⚠️ **Усе нижче про `create-ttn`/`reset-ttn` і дзеркало старих полів описує
Етапи 1-2 і вже НЕ діє** — див. «Посилки — Етап 3».

⚠️ 409 більше не про «є ttn», а про «все розподілено». Друга коробка з рештою
товару — нормальний випадок.

⚠️ `put-order` викидає `parcels` із тіла — той самий захист, що для
`internalNote`.

Міграція: `scripts/migrate-ttn-to-parcels.js --dry-run`, ідемпотентна, старих
полів не стирає.

### Скидання ТТН (2026-09-16)

`POST /adm/orders/:numberOfOrder/reset-ttn`, тіло `{ confirmLocalOnly?: boolean }`.

⚠️ **Тихо скидається лише те, що напевно мертве** — коди 2 (видалено) і 3 (не
знайдено). Жива накладна спершу йде в `InternetDocument.delete`; будь-яка відмова
(в дорозі, невгаданий параметр, мовчання Пошти) повертає `needsConfirm` із
текстом Пошти, і рішення ухвалює менеджер. Стерти номер у себе ≠ скасувати
посилку — різницю магазин відчуває грошима.

⚠️ **Форму параметра `DocumentRefs` звірити не вдалося:** Пошта перевіряє ключ
РАНІШЕ за параметри, і без бойового ключа будь-який `delete` відповідає «User is
undefined» (контрольний трекінг тим самим порожнім ключем при цьому проходить).
Саме тому будь-яка відмова трактується як «не видалено»: помилка в параметрі
коштує зайвого питання менеджеру, а не втраченої посилки. **Перевірити на
першому живому скиданні.**

⚠️ `order.ttnRef` — Ref накладної, зберігається при створенні. У замовлень,
створених раніше, його немає: Ref береться з трекінгу (поле `RefEW`).

⚠️ Скидання стирає `ttn`, `ttnRef` і **три `npStatus*`**. Без останніх крон
порівняв би статус НОВОЇ накладної зі збереженим кодом СТАРОЇ, вирішив «не
змінилось» і замовк назавжди.

⚠️ Статус замовлення, `stockDeducted` і `deliveredAt` НЕ чіпаються: скидання
номера — не скасування відправлення.

### Статус посилки у списку замовлень (2026-09-16)

⚠️ `getOrders` віддає **явний білий список полів**, а не документ цілком. Нове
поле там не з'являється саме собою — саме через це статуси трекінгу спершу були
невидимі в переліку, хоча в базі лежали. `getOrderById` віддає замовлення
цілком, тож у картці вони були одразу.

⚠️ `updateOrderById` ВИДАЛЯЄ `npStatusCode/npStatusText/npStatusUpdatedAt` з тіла:
відколи ці поля є в списку, адмінка шле їх назад при кожній зміні статусу, і без
цього крок «Відправлено → Доставлено» зі списку затирав би те, що Пошта прислала
хвилину тому. Те саме правило, що для `internalNote` й `prepaymentAmount`.

### Автотрекінг статусів Нової Пошти (2026-09-16)

Перша ФОНОВА задача в проєкті. `jobs/scheduler.js` + `jobs/npTrackingJob.js`,
`node-cron` у наявному веб-процесі, `'0 7-21 * * *'`.

⚠️ **Стартує з `server.js`, а НЕ з `app.js`.** `app.js` імпортують тести й будь-які
інструменти, що піднімають застосунок без мережі, — крон ходив би в Пошту з-під
кожного прогону.

⚠️ **`node-cron` закріплений на `^3.0.3`, і підняти його не можна.** 4.x вимагає
Node ≥ 20, а проєкт закріплений на `engines: ^18.*` — Render валить збірку на
етапі `yarn` із «The engine "node" is incompatible». Перед додаванням БУДЬ-ЯКОЇ
залежності звіряйте її `engines` з `engines` проєкту: локальний Node новіший і
ставить те, що на Render не запуститься.

⚠️ 3.x не має `getNextRun()` — виклик неіснуючого методу поклав би планувальник
одразу на старті. У лозі лише розклад і пояс.

⚠️ **`timezone: 'Europe/Kyiv'` задано явно.** Render в UTC (див. коментар у
`helpers/dashboard.js`), і без цього «сьома ранку» була б десятою за Києвом, а
взимку й влітку — у різний час.

⚠️ **Замок обов'язковий** (`helpers/cronLock.js`, `models/cronLock.js`). Крон живе
у веб-процесі, а процес не гарантовано один — досить другого інстансу або
перетину під час деплою, щоб менеджер отримав ДВА однакові сповіщення про
повернення. Береться ОДНИМ атомарним `findOneAndUpdate`: «прочитати й записати»
дало б обом примірникам побачити «вільно» в ту саму мілісекунду. Прострочений
(15 хв) замок перехоплюється — інакше смерть процесу посеред проходу замкнула б
задачу назавжди.

⚠️ **Статус НЕ міняється через `updateOrderById`.** Це express-контролер, і
підробляти йому req/res із крона означало б залежати від того, що він колись не
почне читати щось іще з запиту. Прохід повторює рівно те, що контролер робить на
«Відправлено → Доставлено»: `isTransitionAllowed`, `deliveredAt` один раз, запис
у журнал. **Перевірено:** склад цей перехід не чіпає (списання на «Оплачено»,
повернення на «Скасовано»), листів немає взагалі (`sendEmail` кличуть лише
верифікація пошти й 3D-друк), модалка-подяка — річ адмінки.

⚠️ **Журнал пишеться лише при зміні КОДУ.** Пошта редагує текст статусу сама (там
місто й орієнтовна дата), тож порівняння за текстом додавало б рядок щогодини.

⚠️ Трекінг ходить **без ключа** (`npPost(..., '')`) — метод це дозволяє
(перевірено живим запитом), і щогодинне опитування не витрачає ліміти бойового
ключа, яким створюються накладні.

⚠️ `try/catch` — **на кожну пачку** й **на кожне замовлення** окремо, не навколо
циклу. Одна зіпсована накладна не має позбавляти перевірки решту.

⚠️ Коди статусів, крім `3`, узяті з документації й ще не бачили живої посилки.
Невідомий код НЕ ігнорується — лягає в журнал як проміжний, щоб було видно, що
саме прислала Пошта. Таблиця в `helpers/npTracking.js` коригується за реальними
даними.

⚠️ Тип журналу `np_status` — це статус ПОСИЛКИ, а не замовлення: замовлення
тижнями стоїть у «Відправлено», поки посилка проходить п'ять станів Пошти.

### Внутрішня примітка замовлення (2026-09-16)

`order.internalNote` — одне поточне значення, клієнту не видно.

⚠️ **Не плутати з `order.comment`** — то коментар покупця з оформлення, він його
пише й бачить у кабінеті.

⚠️ Змінює ЛИШЕ `PATCH /adm/orders/:numberOfOrder/note`. `put-order` явно викидає
`internalNote` з тіла: адмінка шле туди весь об'єкт замовлення, і без цього
кожна зміна статусу переписувала б примітку тим, що випадково лежало в сторі.
Той самий клас помилки, що вже був із `prepaymentAmount`.

⚠️ Збереження непорожньої примітки ДОДАЄ запис у журнал — так історія лишається
навіть після того, як поточну стерли. Очищення в журнал не пише: менеджер
прибрав нагадування собі ж, події тут немає.

⚠️ Чому окреме поле, а не «останній запис журналу»: журнал навмисно незмінний,
тож похідна від нього примітка не піддається очищенню в принципі — текст
повертався б на наступному завантаженні сторінки.

### Журнал замовлення (2026-09-15)

`models/orderJournal.js` — ОКРЕМА модель, не поле в CRM-Activity.

⚠️ **Не плутати з `order.comment`** — то коментар КЛІЄНТА з оформлення
(«подзвоніть перед відправкою»), його пише покупець. Журнал внутрішній: записи
менеджера й системи, клієнту не видно ніколи.

⚠️ Чому окрема модель: Activity і запис журналу мають спільну форму, але не суть
(«що людина зробила з людиною» проти «що сталося з документом»). Злиття вимагало б
зробити `contactId` необов'язковим — а це справжній інваріант CRM — і звести типи
в перелік, де половина значень невалідна в кожному контексті. Спільною лишилась
ДИСЦИПЛІНА: `безпечнийЗапис` у `helpers/activities.js` — одна функція на дві
колекції, бо запис у журнал не сміє валити операцію, яку він описує.

⚠️ `POST .../journal` фіксує тип `note`. 'status' і 'ttn' означають «це записала
система», і мітка варта чогось лише доти, доки її не може поставити будь-хто.

⚠️ **Автоподії в `updateOrderById` — рівно два рядки**, після успішного
збереження й ДО `res.json`. Раніше писали б про зміну, якої могло не статись;
пізніше — це робота після відповіді, а такі хвости в цьому контролері вже
коштували помилок.

⚠️ Статус порівнюється з `current` (стан ДО оновлення), а не з тілом запиту:
адмінка шле назад увесь об'єкт, і `status` приходить на кожне збереження.

⚠️ ТТН ловиться за ПОЯВОЮ (`!current.ttn && order.ttn`). Автоформування зберігає
номер окремим запитом і пише свій рядок там, тож тут дубля не буде; ця перевірка
працює лише для номера, введеного руками.

### Хронологія активностей (2026-09-15)

`models/activity.js` — ОКРЕМА колекція, не піддокумент Contact: у активного
клієнта записів десятки, і масивом вони їхали б у кожній відповіді зі списком
клієнтів.

⚠️ Журнал НЕ редагується й не видаляється. Хронологія, яку можна підчистити, не
варта нічого; помилковий запис виправляється наступним записом.

⚠️ `POST /adm/contacts/:id/activities` не приймає тип `system`: позначка
«записала система» щось означає лише доти, доки її не може поставити будь-хто.

⚠️ **`nextContactDate` у тілі має ТРИ стани**: поля немає — не чіпати; `null` —
зняти; дата — виставити. Без цієї різниці звичайна нотатка зносила б домовленість
передзвонити.

Автоподії: замовлення (ПІСЛЯ `Order.create` у трьох контролерах — інакше рядок
описував би замовлення, яке могло не створитись), зміна стадії, вхід у Реанімацію.
Тексти зібрані в `helpers/activities.js` — інакше та сама подія записувалась би
трьома формулюваннями.

### Нагадування (2026-09-15)

`GET /adm/funnel` віддає `{ result, reminders }` — блок живе вгорі тієї самої
сторінки. `GET /adm/counters` отримав `reminders`.

⚠️ Лічильник `reminders` — НЕ «непереглянуте». Решта знімається кнопкою й
зменшується в адмінці локально; цей — обчислюваний стан, тож `markViewed` до
нього немає і віднімати локально не можна.

⚠️ У нагадування потрапляють і ті, кого на дошці немає: клієнт міг купити й вийти
з воронки, поки домовленість передзвонити лишалась.

### Воронка продажів (2026-09-14)

`GET /adm/funnel` — дошка, згрупована за стадіями. `PATCH /adm/contacts/:id/stage`
— зміна стадії. Стадії й правило участі — `helpers/funnel.js`.

⚠️ **У воронці лише `type==='manual'` АБО `kind==='wholesale'`.** Роздрібних із
сайту тисячі, вони купують самі й ні з ким не домовляються — дошка з ними стала б
другим, гіршим списком клієнтів. Правило живе в ОДНОМУ файлі, бо його потребують
три місця (ендпоінт дошки, зміна стадії, скрипт); друга копія розійшлася б, і
клієнт, якого скрипт вважає учасником, зник би з дошки.

⚠️ **Хто входить у воронку із замовленнями — одразу в «Співпрацюємо»**, не в
«Новий лід». Діє ЗАВЖДИ, не лише в скрипті: роздрібний, якого позначили
оптовиком, — не лід. Момент входу ловиться порівнянням `isInFunnel` до і після
правки; редагування клієнта, який УЖЕ у воронці, стадію не чіпає — виправлення
друкарської помилки в імені не має відкидати його на початок.

⚠️ **`lostReason` і `nextContactDate` живуть рівно поки стадія `lost`.** Повернули
в роботу — обидва чистяться: «відмовив, бо дорого» біля клієнта, з яким знову
ведуть перемовини, дезінформує.

⚠️ Суми замовлень для дошки — ОДНИМ запитом (`loadOrdersTotals`), а не запитом на
клієнта як у `getContacts`. Там сторінка на 50; тут уся воронка, і сотня
оптовиків означала б сотню запитів на кожне відкриття вкладки.

`scripts/init-funnel-stages.js --dry-run` → без прапорця. Ідемпотентний, чіпає
лише тих, у кого стадія досі `new`.

### Пошта: що надсилається, а що ні (2026-09-10)

Клієнту НЕ шлються листи про замовлення. Прибрано два:
`controllers/orders.js` («Ваше замовлення №… оформлено») і
`controllers/acquiring.js` («замовлення прийнято, очікуємо оплату»).
Рішення власника: про замовлення клієнта інформує менеджер особисто, а магазин
дізнається з Telegram (`notifyNewOrder`) і з адмінки.

Поштою лишаються **тільки**:
- підтвердження адреси при реєстрації — `controllers/auth.js` (×2),
  `controllers/user.js`;
- ⚠️ **лист про замовлення на 3D-друк** — `controllers/print3d.js`. Він іде
  НЕ клієнту, а магазину самому собі (`from: MAIL_USER, to: MAIL_USER`), і
  несе завантажений файл ВКЛАДЕННЯМ. У базі 3D-замовлення файл не зберігається
  взагалі: multer пише його на диск, лист забирає, а далі `fs.unlink` видаляє.
  Вимкнути цей лист = мовчки втрачати файли клієнтів. Спершу треба навчитись
  зберігати файл (Cloudinary чи диск), і лише тоді чіпати пошту.

Не повертати листи клієнту, не спитавши власника.

### CRM: клієнти (Contact) — Етап 1 (2026-09-14)

Нова сутність `Contact` — клієнт CRM. Замовлення прив'язується до нього полем
`Order.contactId`.

⚠️ **`Contact` — це НЕ `User`.** `User` — акаунт на сайті, він є лише в тих, хто
реєструвався. `Contact` охоплює ВСІХ, хто замовляв (переважно гості без
акаунта), плюс доданих руками. В адмінці вони живуть двома вкладками одного
розділу «Клієнти». Не зливати їх в одну сутність.

⚠️ **Пошук клієнта — ТІЛЬКИ за нормалізованими цифрами** (`phones.digits`).
Один номер лежить у базі як `+380671234567`, `0671234567` і
`+38 (067) 123-45-67`; порівняння рядків завело б окремого клієнта на кожне
написання. Нормалізатор спільний із Новою Поштою — `helpers/phone.js`. Двох
копій не заводити: розбіжність прив'яже замовлення не до тієї людини, і
помітять це дуже пізно.

⚠️ **Авто-прив'язка стоїть у ТРЬОХ місцях** — `orders.js`, `monopay.js`,
`acquiring.js`. Замовлення народжується в кожному з них; забути одне означає
мовчки втратити частину клієнтів. Логіка — в одному хелпері
`helpers/contacts.js`, свідомо БЕЗ Mongoose-хука.

⚠️ **Збій прив'язки не валить замовлення** (`attachContactToOrder` ковтає
помилку й повертає `null`). CRM внутрішня; клієнт не має бачити помилку
оформлення через те, що не записався запис для менеджера.

⚠️ **Картка клієнта шукає замовлення за `contactId` АБО за телефоном.** Усе,
замовлене до появи CRM, `contactId` не має, і без другої умови історія
постійного клієнта була б порожня. У відповіді кожне замовлення має `linked` —
чи прив'язане воно явно.

⚠️ **Видалення клієнта відв'язує замовлення, а не видаляє їх.**

⚠️ **`Order` вимагає `tel`, `firstName`, `lastName`, `email`** — тож запобіжники
в хелпері (порожній телефон, порожнє ім'я) через оформлення недосяжні. Вони для
скрипта й ручних правок у базі.

Разовий скрипт: `node scripts/init-contacts.js --dry-run`, потім без прапорця.
Ідемпотентний, ручні прив'язки не перебиває, ім'я бере з найсвіжішого
замовлення номера.

### ТТН Нової Пошти (додано 2026-09-10)

`POST /api/adm/orders/:numberOfOrder/create-ttn` → `InternetDocument.save`.
Уся робота з API — у `helpers/novaposhta.js`, збірка накладної — в `helpers/ttn.js`.

⚠️ **Нова Пошта відповідає HTTP 200 навіть коли відхиляє запит** — вердикт лежить
у тілі, в `success`. Тому axios на її ділових помилках НЕ кидає винятку, і
`.catch()` навколо виклику не спрацьовує взагалі. Старий код у `getDeliveryCity`
саме так і мовчав роками: відмова Пошти доїжджала до клієнта як порожній список
міст. Перевіряти `success` — обов'язково.

⚠️ **Шлях у URL Пошта ігнорує.** Маршрутизація йде тільки за `modelName` +
`calledMethod` у тілі. Старий код постив на `/searchSettlements`, маючи в тілі
`getCities` — тобто роками працював `getCities`, попри назву. Перевірено:
запит на `/anything-at-all` повертає те саме.

⚠️ **`ServiceType` не можна зашивати як `WarehouseWarehouse`.** У списку, який
бачить клієнт на checkout, переважна більшість адрес — поштомати (у Львові 2809
із 2967), і для них потрібен `WarehousePostomat`. Визначається автоматично з
`CategoryOfWarehouse`, який приходить тим самим запитом, що й Ref відділення.
Звідти ж ліміт ваги: поштомат 20 кг проти 1100 кг у відділення.

⚠️ **Адреса отримувача відновлюється з ТЕКСТУ.** Ref у замовленні ніколи не
зберігався (checkout пише лише назви), але назви Пошти однозначні: однойменні
міста несуть район у самій назві, а назви відділень унікальні в межах міста.
Тому ТТН можна робити й для старих замовлень. Checkout чіпати не довелось.

⚠️ **Статус замовлення `create-ttn` НЕ змінює.** Перехід у «Відправлено» має
власні правила (`updateOrderById`), і другий шлях зміни статусу неминуче
розійшовся б із першим. Адмінка після успіху викликає той самий
`updateOrderById`, що й при ручному вводі номера.

⚠️ **`OptionsSeat` обовʼязковий навіть для однієї коробки.** Без нього
`InternetDocument.save` відповідає «param OptionsSeat required» (код
20000201812) — самих `Weight` і `VolumeGeneral` Пошті не досить. Тому в модалці
питаються ГАБАРИТИ в сантиметрах, а обʼєм рахується з них. Вага ділиться між
місцями порівну, залишок від округлення йде на останнє місце, щоб сума ваг
місць точно збігалася із загальною.

⚠️ **Накладений платіж іде через «Контроль оплати» (`AfterpaymentOnGoodsCost`),
а НЕ через післяплату.** Це дві різні послуги Пошти:

| | Післяплата | Контроль оплати |
|---|---|---|
| поле | `BackwardDeliveryData[{CargoType:'Money'}]` | `AfterpaymentOnGoodsCost` (число) |
| гроші | переказ, який забирають у відділенні | на банківський рахунок магазину |
| договір | з НП | з НП **і NovaPay** |

Обрано Контроль оплати — рішення власника: гроші на рахунку краще за готівку у
відділенні. **Обидва поля разом не шлються.**

⚠️ Відмова по накладеному платежу — це не про payload, а про договір: звичайна
накладна з тими самими даними створюється, ламається лише накладений платіж.
Код питає `Counterparty.getCounterpartyOptions` → `CanAfterpaymentOnGoodsCost`,
щоб сказати це менеджеру прямо; перевірка НЕобов'язкова — якщо впаде,
повідомлення лишиться загальнішим. Обхід без послуги: наложка 0 → накладна
створюється без цього поля.

⚠️ **Неперевірене:** чи потрібен `BackwardDeliveryData` ПОРУЧ із
`AfterpaymentOnGoodsCost`. Джерела розходяться, офіційна сторінка методу
віддає 403, а живим викликом без підключеної послуги не перевірити. Якщо Пошта
відмовить після підключення — дивитись її текст.

⚠️ **Номер зберігається одразу після відповіді Пошти, до всього іншого.**
Накладна, що існує в Пошті й ніде в нас, — це оплачена посилка, про яку магазин
не знає, а другу для того самого замовлення вже не створити (`create-ttn`
віддає 409, якщо `ttn` уже є).

Дані відправника — у `Settings.npSender`. Контрагент і контактна особа
зберігаються як Ref + людська назва: це внутрішні id кабінету, яких не вивести
з назви, тож адмінка бере їх списком через `GET /api/adm/np/senders`.

⚠️ **Місто й відділення відправника зберігаються ТІЛЬКИ назвою** — Ref для них
резолвиться при створенні ТТН, тим самим механізмом, що й для отримувача.
Спершу в схемі були ще й `cityRef`/`warehouseRef`, але заповнювати їх не мав
хто: адмінка обирає адресу з довідника й пише назву. Виходило, що повністю
заповнені налаштування все одно давали «не заповнено місто відправлення».
Поля прибрані — порожнє поле, яке виглядає як ідентифікатор, гірше за його
відсутність.


⚠️ **`GET /api/products/:id` шукає товар за `codeOfGood`, а не за `_id`**
(`controllers/products.js` → `findOne({ codeOfGood: id })`). Адмінка в маршруті
`admin/assortment/:cardId` мусить передавати саме артикул; підстановка `_id`
мовчки відкриває чужий товар або нічого. Той самий контролер має ще й
пре-існуючу ваду: знайшовши `product`, він викликає `res.json` і **не
повертається** — далі йде другий `res.json` для збірки, і Express кидає
`ERR_HTTP_HEADERS_SENT` у лог. Клієнт при цьому вже отримав правильну
відповідь, тож симптом лише в логах.
...
- **`req.rawBody`.** У `app.js`: `express.json({ verify: (req,res,buf) => { req.rawBody = buf } })`
  ...
  роутів.
- **Окремі axios-клієнти — принципово.** Кожна зовнішня інтеграція має власний
  `axios.create({...})`: `helpers/novaposhta.js`, `helpers/monopay.js`, `helpers/acquiring.js`,
  `helpers/telegram.js`. Глобальний `axios.defaults.baseURL`, який `controllers/orders.js` тримав
  зайнятим Новою Поштою, **прибрано 2026-09-10** — саме через нього три модулі й змушені були
  заводити приватні клієнти. Не повертати.
