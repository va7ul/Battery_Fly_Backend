...
Дані відправника — у `Settings.npSender` (Ref + людська назва парою). Контрагент
і контактна особа — єдине, чого не вивести з назви: це внутрішні id кабінету,
тож адмінка бере їх списком через `GET /api/adm/np/senders`.
