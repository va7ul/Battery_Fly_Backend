...
⚠️ **Дзеркало розширене на статус.** Відколи трекінг пише статус у посилку,
`order.npStatus*` ніхто б не оновлював — а на них спирається пігулка в СПИСКУ
замовлень, і вона замерзла б на тому, що встиг записати старий крон. Тепер
`syncLegacyTtn` дзеркалить і статус ПЕРШОЇ посилки. Приберемо в Етапі 3.
...
⚠️ **Старі `ttn`/`ttnRef`/`npStatus*` НЕ прибрані** — це дзеркало ПЕРШОЇ посилки
(`syncLegacyTtn`). На них спираються крон трекінгу (Етап 2) та `{ttn}` у шаблонах
і Telegram (Етап 3). Прибрати зараз = мовчки зламати відстеження й листування.

⚠️ **Деплой НЕ мусить бути синхронним.** `create-ttn` і `reset-ttn` лишились
робочими й керують першою посилкою; `create-ttn` пакує все нерозподілене. Ручний
ввід номера через `put-order` теж створює посилку — інакше стара адмінка робила
б ТТН повз облік.

⚠️ `reset-ttn` заводить посилку НА ЛЬОТУ для замовлень ДО міграції: у них є
номер, але немає посилок, і без цього кнопка мовчки нічого б не робила рівно на
тих замовленнях, заради яких її натискають.
...
`internalNote` і `npStatus*`.

⚠️ **Обмеження Етапу 1:** крон читає `order.ttn`, тобто відстежує лише ПЕРШУ
посилку. Решта — без статусу до Етапу 2.
