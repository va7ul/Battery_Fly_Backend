...
`period` = `today` | `week` | `month` (дефолт) | `custom` (+`from`/`to`).
...
- **Попередній період зсувається на ЦІЛУ кількість діб** (1/7/30), а не на
...
- **Оптовик** = >3 замовлень і >50 000 ₴ за 30 днів (обидві умови).
  ⚠️ Для `topCustomers` вікно — останні 30 днів. Для `inactiveCustomers` —
  30 днів **до останнього замовлення клієнта**. Інакше список був би
  структурно порожній: хто не купував 40 днів, не може мати 3+ замовлень за
  останні 30. Питання там — «чи був він оптовиком, поки був активний».
