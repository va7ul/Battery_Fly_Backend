...
⚠️ **Накладений платіж потребує ПОСЛУГИ на договорі з Поштою.** «Передана
послуга Післяплата недоступна» (код 20000204637) — це не про payload:
`CargoType: Money` і `PayerType: Recipient` беруться з довідників самої Пошти
(`getBackwardDeliveryCargoTypes`, `getTypesOfPayersForRedelivery`), і звичайна
накладна з тими самими даними створюється. Післяплату вмикає менеджер НП.
Код питає `Counterparty.getCounterpartyOptions` →
`CanAfterpaymentOnGoodsCost`, щоб сказати це менеджеру прямо; перевірка
НЕобов'язкова — якщо впаде, повідомлення просто лишиться загальнішим.
Обхід без послуги: наложка 0 → накладна створюється без `BackwardDeliveryData`.
