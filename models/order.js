...
const orderSchema = new Schema(
    {   
        ...
        // ⚠️ DEPRECATED — дзеркало ПЕРШОЇ посилки, не джерело істини.
        //
        // Лишається доти, доки на order.ttn спираються: крон трекінгу (Етап 2),
        // {ttn} у шаблонах повідомлень клієнту й Telegram (Етап 3). Прибрати
        // ці поля зараз означало б мовчки зламати відстеження й листування з
        // клієнтом у проді — тому синхронізуємо їх із parcels[0] і знімемо
        // разом з Етапом 3.
        //
        // Номер ТТН. Вводиться менеджером при переході в "Відправлено".
        ttn: {
            type: String,
            default: null,
        },
        // Ref накладної в Новій Пошті — її внутрішній ідентифікатор (GUID).
        //
        // ⚠️ Не те саме, що номер: номер друкують на наліпці й показують
        // клієнту, а Ref потрібен методам, які накладну ЗМІНЮЮТЬ — зокрема
        // InternetDocument.delete. Пошта повертала його при створенні й
        // раніше, але ми його не зберігали: до появи скидання ТТН він нікому
        // не був потрібен.
        //
        // ⚠️ У замовлень, створених ДО цього поля, тут null. Для них Ref
        // дістається з трекінгу — у відповіді getStatusDocuments він лежить у
        // полі RefEW (див. resetOrderTtn).
        ttnRef: {
            type: String,
            default: null,
        },
        // Останній відомий статус посилки в Новій Пошті.
        //
        // ⚠️ Зберігаємо КОД, а не лише текст: саме за кодом крон розуміє, чи
        // статус змінився з минулого проходу. Текст Пошта міняє й сама — у
        // ньому назва міста та орієнтовна дата, — тож порівняння за текстом
        // писало б у журнал на кожному проході, хоча насправді нічого не
        // сталося.
        npStatusCode: {
            type: String,
            default: null,
